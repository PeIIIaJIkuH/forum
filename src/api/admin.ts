...
const dismissModeratorRequest = (requestId: number): ApiPromise =>
	defaultAxios.delete(`admin/request/dismiss/${requestId}`)
...
const dismissPostReport = (reportId: number): ApiPromise => defaultAxios.delete(`admin/post/report/dismiss/${reportId}`)
...
const createCategory = (categoryName: string): ApiPromise =>
	defaultAxios.post('admin/category/add', {
		name: categoryName,
	})
